...
ADD letheanSDP /usr/bin/letheanSDP
ADD _letheanSDP /usr/local/share/zsh/site-functions/_letheanSDP
ADD letheanSDP.bash-completion /etc/bash-completion.d/letheanSDP
RUN chmod 755 /usr/bin/letheanSDP
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/letheanSDP\n\
' >> ~/.bashrc

#
# Setup prompt
#
RUN echo 'export PS1="[Lethean Binary API] \$ "' >> ~/.bashrc
RUN echo 'export PROMPT="[Lethean Binary API] \$ "' >> ~/.zshrc
...
line REST client for $(tput setaf 6)Lethean Binary API$(tput sgr0).\n\
\n\
For convenience, you can export the following environment variables:\n\
\n\
$(tput setaf 3)https://sdp.lt.hn$(tput sgr0) - server URL, e.g. https://example.com:8080\n\
\n\
